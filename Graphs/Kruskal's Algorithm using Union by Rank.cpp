// Finding Minimum Spanning Tree by using Union by Rank & Path Compression || Time Complexity = ElogE
...
void Kruskal(Edge *input, Edge *output, int v)
{
    ...
    while (count < v - 1)
    {
        ...
        if (destination_parent != source_parent)
        {
            ...
            {
                swap(source_parent, destination_parent);
            }
            parent[destination_parent] = source_parent;
            if (rank[destination_parent] == rank[source_parent])
            {
                rank[source_parent]++;
            }
            i++;
